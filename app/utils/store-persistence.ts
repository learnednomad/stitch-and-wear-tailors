...
const AppointmentStoreSnapshotSchema = z.object({
  ...
  bookingForm: z.object({
    ...
    type: z
      .enum(["consultation", "measurement", "fitting", "delivery", "alteration", "design_review"])
      .nullable(),
    selectedSlot: z
      .object({
        startTime: z.string(),
...
export class StorePersistenceManager {
  ...
  static async getStorageStats(): Promise<{
    ...
    try {
      ...
      const storeKeys = allKeys.filter((key) => Object.values(STORAGE_KEYS).includes(key as any))

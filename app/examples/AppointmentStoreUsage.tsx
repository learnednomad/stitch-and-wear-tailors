...
import { createAppointmentStoreEnhanced, AppointmentStoreEnhanced } from '../models/stores/AppointmentStoreEnhanced';
...
const AppointmentStoreExample = observer(() => {
  ...
  const handleCreateAppointment = async () => {
    ...
    if (result.success) {
      ...
      Alert.alert('Validation Error', `Failed to create appointment: ${result.errors?.[0]?.message}`);
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 20 }}>
      ...
      <View style={{ marginBottom: 20, padding: 10, backgroundColor: '#f0f0f0' }}>
        ...
        <Text>Last Fetched: {store.lastFetched ? new Date(store.lastFetched).toLocaleString() : 'Never'}</Text>
      </View>

      {/* Action Buttons */}
      <View style={{ gap: 10 }}>
        <Button
          title="Create Valid Appointment"
          onPress={handleCreateAppointment}
        />
        
        <Button
          title="Load Appointments from API"
          onPress={handleLoadAppointments}
        />

...
const ServicesModel = types.model("Services", {
  ...
  additionalServices: types.array(
...
const AvailabilityModel = types.model("Availability", {
  ...
  bookedSlots: types.array(
    types.model("BookedSlot", {
...
export const AppointmentStoreEnhancedModel = types
            ...
            try {
              ...
              if (error instanceof z.ZodError) {
                validationErrors.push(
                  `Appointment ${appointmentData.id}: ${error.errors[0]?.message}`,
                )
              }
            }
          }
  ...
  .views((self) => ({
    ...
    get todaysAppointments() {
      ...
      return self.appointments.items.filter(
        (apt: any) => apt.scheduledAt.startsWith(today) && apt.status !== "cancelled",
      )
    },
    ...
    get currentAvailability() {
      ...
      return self.availability.items.find(
        (item: any) => item.tailorId === self.selectedTailorId && item.date === self.selectedDate,
      )
    },
...
export interface AppointmentStoreEnhancedSnapshot
  extends SnapshotOut<typeof AppointmentStoreEnhancedModel> {}

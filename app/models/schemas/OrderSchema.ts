...
export const OrderStatusSchema = z.enum([
  'draft', 'pending', 'confirmed', 'measuring', 'cutting', 
  'stitching', 'fitting', 'finishing', 'ready', 'delivered', 
  'cancelled', 'refunded'
])
...
export const CreateOrderInputSchema = z.object({
  clientId: z.string().uuid('Invalid client ID format'),
  tailorId: z.string().uuid('Invalid tailor ID format').optional(),
  items: z.array(OrderItemSchema.omit({ id: true, totalPrice: true })).min(1, 'Order must have at least one item'),

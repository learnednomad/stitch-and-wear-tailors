 ...
 * Comprehensive notification management interface for clients
 */
...
interface ClientNotificationsScreenProps extends AppStackScreenProps<"ClientNotifications"> {}

export const ClientNotificationsScreen: FC<ClientNotificationsScreenProps> = observer(
  ({ navigation }) => {
    const { notificationStore, authStore } = useStores()
    const [refreshing, setRefreshing] = useState(false)
    const [selectedFilter, setSelectedFilter] = useState<"all" | "unread" | "orders">("all")
    ...
    const loadNotifications = async () => {
      try {
        ...
        if (selectedFilter === "orders") params.type = ["order_update", "delivery_notification"]
    ...
    const handleNotificationPress = useCallback((notification: any) => {
      ...
      if (notification.actionUrl) {
        ...
        } else if (notification.actionUrl.includes("/promotions/")) {
          const promotionId = notification.actionUrl.split("/promotions/")[1]
          // Navigate to promotion detail when implemented
    ...
    const getNotificationIcon = (type: string): string => {
      switch (type) {
        ...
        case "promotional_offer": return "gift"
    ...
    const filteredNotifications = notificationStore.notifications.items.filter((notification: any) => {
      ...
      switch (selectedFilter) {
        ...
        case "orders": return ["order_update", "delivery_notification"].includes(notification.type)
        default: return true
      }
    })
      ...
      >
        <View style={$notificationHeader}>
          ...
          <View style={$notificationContent}>
            ...
            <Text style={$notificationBody} numberOfLines={3}>
              {notification.body}
            </Text>
        ...
        {notification.type === "promotional_offer" && (
          <View style={$promotionalBanner}>
            <Icon icon="gift" size={16} color={colors.palette.sageGreen} />
            <Text style={$promotionalText}>Special Offer</Text>
          </View>
        )}
        ...
        {showSettings && (
          <View style={$settingsPanel}>
            <View style={$settingsPanelContent}>
              ...
              <TouchableOpacity style={$settingItem}>
                <Text style={$settingLabel}>Promotional Offers</Text>
                <View style={$settingValue}>
                  <Text style={$settingValueText}>
                    {notificationStore.preferences?.promotionalOffers ? "On" : "Off"}
                  </Text>
                  <Icon icon="caretRight" size={16} color={colors.palette.neutral400} />
                </View>
              </TouchableOpacity>
...
// Styles (reusing most from Tailor screen with client-specific modifications)
...
const $unreadCard: ViewStyle = {
  ...
  backgroundColor: colors.palette.threadBlue + "05",
}
...
const $promotionalBanner: ViewStyle = {
  flexDirection: "row",
  alignItems: "center",
  gap: spacing.xs,
  ...
  borderTopWidth: 1,
  borderTopColor: colors.palette.sageGreen + "20",
}

const $promotionalText: TextStyle = {
  fontSize: 12,
  fontWeight: "600",
